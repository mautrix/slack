...
func ParseMediaID(mediaID networkid.MediaID) (any, error) {
	...
	case MediaIDTypeFile:
		userLoginID, mediaID, err := readShortString(mediaID)
		if err != nil {
			return nil, fmt.Errorf("failed to read user login ID: %w", err)
		}
		fileID, _, err := readShortString(mediaID)
		if err != nil {
			return nil, fmt.Errorf("failed to read file ID: %w", err)
		}
		return &DirectMediaFile{
			UserLoginID: networkid.UserLoginID(userLoginID),
			FileID:      fileID,
		}, nil
	default:
...
func (dme *DirectMediaEmoji) MediaID() networkid.MediaID {
	...
	buf := []byte{byte(MediaIDTypeEmoji)}
	buf = appendShortString(buf, dme.TeamID)
	buf = appendShortString(buf, dme.EmojiID)
	buf = appendShortString(buf, dme.FileName)
	return buf
}

type DirectMediaFile struct {
	UserLoginID networkid.UserLoginID
	FileID      string
}

func (dmf *DirectMediaFile) MediaID() networkid.MediaID {
	if dmf == nil || len(dmf.UserLoginID) > 255 || len(dmf.FileID) > 255 {
		return nil
	}
	buf := []byte{byte(MediaIDTypeFile)}
	buf = appendShortString(buf, string(dmf.UserLoginID))
	buf = appendShortString(buf, dmf.FileID)
	return buf
}

func appendShortString(buf []byte, value string) []byte {
	return slices.Concat(buf, []byte{byte(len(value))}, []byte(value))
}
